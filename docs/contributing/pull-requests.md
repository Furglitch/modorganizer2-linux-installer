...
1. Ensure your changes pass all pre-commit checks (these run automatically if you've set up pre-commit hooks. See [Development Environment](./dev-environment.html)).
...
5. If adding a new game, ensure the game info entry is added to `configs/game_info.yml` following [Configuration Files](./configuration-files.html). See [Adding a New Game](./adding-a-game.html).
